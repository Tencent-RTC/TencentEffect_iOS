...
+ (void)addObserver:(id)observer forGroupId:(NSString *)groupId
{
    ...
    if ([groupId length] > 0 && observer)
    {
        ...
        [rb_observers setObject:weakObserver forKey:groupId];
...
{
    [self.textLbl setFont:font];
}

- (void)setTextColor:(UIColor *)color
{
    [self.textLbl setTextColor:color];
}
...
//        id observer = [rb_observers objectForKey:self.groupId];
        NSValue *weakObserver = [rb_observers objectForKey:self.groupId];
        id observer = [weakObserver nonretainedObjectValue];
